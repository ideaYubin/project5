 ...
 * CURR_IAMGE_REPO_PATH = "c:\\spring\\image_repo";
 * 
 ...
 * response.getOutputStream(); String filePath = CURR_IAMGE_REPO_PATH + "\\" +
 * imageFileName; File image = new File(filePath); int lastIndex =
 * imageFileName.lastIndexOf("."); String fileName = imageFileName.substring(0,
 * lastIndex); File thumbnail = new File(CURR_IAMGE_REPO_PATH +
 * "\\" + "thumbnail" + "\\" + fileName + ".png"); if(image.exists()) {
 * thumbnail.getParentFile().mkdirs(); Thumbnails.of(image).size(50,
 * 50).outputFormat("png").toFile(thumbnail); }
 * 
 * 
